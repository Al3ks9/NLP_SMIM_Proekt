...
def assemble_prompt(source_poem_text: str, source_author: str, source_title: str,
                    target_author: str, model: str = DEFAULT_MODEL,
    ...
    content_keywords = poem_tfidf.top_content_words(source_author, source_title, n=n_keywords)
...
def load_poem_text(author: str, song_title: str) -> str:
    """
    A poem's text by (author, song_title).
    ...
    'ПЕСНА'; see CLAUDE.md's Data quality context). Silently picking one on
    collision would make load_poem_text return a plausible-looking but wrong
    poem, so an ambiguous title raises instead of guessing.
    """
    def build():
        by_key = defaultdict(list)
        with open(DATA / 'stripped_songs.csv', encoding='utf-8') as f:
            for r in csv.DictReader(f):
                by_key[(r['author'], r['song_title'])].append(r['song_text'])
        return dict(by_key)

    rows = _cached('stripped_songs_by_key', build)
    matches = rows.get((author, song_title))
    if not matches:
    ...
    if len(matches) > 1:
        raise ValueError(
            ...
            "stripped_songs.csv — (author, song_title) isn't a unique poem key here, and "
            'this tool has no way to disambiguate by title alone. Pick a different poem, '
            'or resolve the duplicate upstream.'
        )
    return matches[0]
...
def run_transfer(source_author: str, source_title: str, target_author: str,
                 ...
                 instr_lang: str = DEFAULT_INSTR_LANG, **assemble_kw) -> dict:
    """
    Full step 1-9 chain for one source poem -> target author transfer: loads the
    source poem, assembles the prompt, generates the poem, measures structural
    fit, logs the run to LOG_DIR, and returns everything for the caller.
    """
    source_text = load_poem_text(source_author, source_title)
    assembled = assemble_prompt(source_text, source_author, source_title, target_author,
                                model=model, backend=backend, instr_lang=instr_lang,
                                **assemble_kw)
    ...
    log.info('transfer source=%s (%s) -> target=%s  lines=%d/%d stanzas=%d/%d  log=%s',
            source_author, source_title, target_author,
...
if __name__ == '__main__':
    ...
    if not args.source_title:
        # No title given: pick this author's first poem in the corpus.
        with open(DATA / 'stripped_songs.csv', encoding='utf-8') as f:
